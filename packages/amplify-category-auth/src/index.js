...
async function externalAuthEnable(context, externalCategory, resourceName, requirements) {
  const { amplify } = context;
  const serviceMetadata = amplify.readJsonFile(`${__dirname}/../templates/supported-services.json`);
...
async function console(context) {
  const { amplify } = context;
  const supportedServices = amplify.readJsonFile(`${__dirname}/../templates/supported-services.json`);
