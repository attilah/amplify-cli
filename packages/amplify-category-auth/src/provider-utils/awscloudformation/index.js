...
async function copyCfnTemplate(context, category, options, cfnFilename) {
  ...
  const copyJobs = [
    {
      dir: pluginDir,
      template: `../../../templates/cloudformation-templates/${cfnFilename}`,
...
async function addResource(context, category, service) {
  let props = {};
  serviceMetadata = context.amplify.readJsonFile(`${__dirname}/../../../templates/supported-services.json`)[service];
...
async function updateConfigOnEnvInit(context, category, service) {
  const srvcMetaData = context.amplify.readJsonFile(`${__dirname}/../../../templates/supported-services.json`).Cognito;
...
async function migrate(context) {
  ...
  const servicesMetadata = amplify.readJsonFile(`${__dirname}/../../../templates/supported-services.json`);
...
async function copyS3Assets(context, props) {
  ...
  if (confirmationFileNeeded) {
    if (!existsSync(targetDir)) {
      const source = `${__dirname}/../../../templates/triggers/CustomMessage/assets`;
...
async function createAdminAuthFunction(context, authResourceName, functionName, adminGroup, operation) {
  ...
  const copyJobs = [
    {
      dir: pluginDir,
      template: '../../../templates/adminAuth/admin-auth-app.js',
      target: `${targetDir}/function/${functionName}/src/app.js`,
    },
    {
      dir: pluginDir,
      template: '../../../templates/adminAuth/admin-auth-cognitoActions.js',
      target: `${targetDir}/function/${functionName}/src/cognitoActions.js`,
    },
    {
      dir: pluginDir,
      template: '../../../templates/adminAuth/admin-auth-index.js',
      target: `${targetDir}/function/${functionName}/src/index.js`,
    },
    {
      dir: pluginDir,
      template: '../../../templates/adminAuth/admin-auth-package.json',
      target: `${targetDir}/function/${functionName}/src/package.json`,
    },
    {
      dir: pluginDir,
      template: '../../../templates/adminAuth/admin-queries-function-template.json.ejs',
...
async function createAdminAuthAPI(context, authResourceName, functionName, operation) {
  ...
  const copyJobs = [
    {
      dir: pluginDir,
      template: '../../../templates/adminAuth/admin-queries-api-template.json.ejs',
      target: `${targetDir}/api/${apiName}/admin-queries-cloudformation-template.json`,
    },
    {
      dir: pluginDir,
      template: '../../../templates/adminAuth/admin-queries-api-params.json',
